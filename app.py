...
from wind_math import (bearing, circular_ema, circular_mean_columns,
                       circular_sma, distance_m, held, midpoint, shade,
                       signed_diff, signed_diff_series, sustained_extremes)
...
def fetch_live(now: pd.Timestamp, sma: pd.Timedelta):
    ...
    return wd.parallel(
        lambda: wd.fetch_wind(now - _lookback(sma), now),
...
def _replay_block(block_start: pd.Timestamp, sma_s: int):
    ...
    (twd, tws), boat, pos = wd.parallel(
        lambda: wd.fetch_wind(start, end),
...
def draw():
    ...
    st.markdown(render(r, rng, compute_boat(boat), sma_min)
                + status(r, tag, vnow), unsafe_allow_html=True)
